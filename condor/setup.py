...
ANA_BASE = os.environ['CMSSW_BASE']+'/src/ttB'
if opt == "unskim":
    ...
    with open(flist) as filelist:
        for line in filelist:
            ...
            if re.compile('.*20[1-2][0-9][A-J].*').match(sample):
                ...
                print>>fl, filename
            # Signals
            elif re.compile('.*T[1-9][t,b,c,q,W,Z,H][t,b,c,q,W,Z,H].*').match(sample) or "ttbarToBsToTauTau" in sample or "SMS-" in sample:
